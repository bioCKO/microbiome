...
=head1 Usage:
    ...
    --step          [str]   analysis step as follow, default=123
                            ...
                            3.diversity analysis (1: TopN heatmap or bar drawing. 2:LefSe. 3:GraPhlAn 4: Wilcox test) 
...
[options for step3]
    ...
    --top_bar       [str]   set top N species number for bar drawing.
    --top_heatmap   [str]   set top N species number for heatmap.
...
my %opt = (
    ...
    step=>'123',
    #step1
    fqp=>'',adp=>'',rf_opts=>'',qc_options=>'',hvf=>'8G',host=>\@host,
    #step3
    top_heatmap=>'35',top_bar=>'10',
    #others
    AVF=>'8G/7G;5G/1G',outdir=>".",shdir=>"./Shell",
);
GetOptions(
   \%opt, 
   #main
   "step:s",
   #step1
   "data_list:s","read_len:s","group:s","fqp:s","adp:s","rf_opts:s","host:s","hvf:s","qc_options:s",
   #step2
   "reads_list:s",
   #step3
   "relative_dir:s","top_bar:n","top_heatmap:n","mf:s","lefse_vs:s",
   #others
   "AVF:s","outdir:s","shdir:s","help:s","notrun","locate",
);
#end for get options
...
my $lib = "$Bin/../lib/";
use PATHWAY;
(-s "$Bin/Pathway_cfg.ori.txt") || die"error: can't find config at $Bin/, $!\n";
my ($super_worker, $qc, $metaphlan, $diversity, $sh_control) = get_pathway("$Bin/Pathway_cfg.ori.txt",[qw(SUPER_WORK QC_MAIN METAPHLAN DIVERSITY SH_CONTRAL2)]);
...
if($opt{step}=~/1/){
    ...
    shell_box(*SH,"1) run Data Clean","step1.DataClean.sh");
...
if($opt{step}=~/2/){
    ...
    shell_box(*SH, "2) run metaphlan taxAnno", "step2.MetaPhlAn.sh");
    $diversity .= " --relative_dir $dir[1]/03.Stat/Relative";
}
