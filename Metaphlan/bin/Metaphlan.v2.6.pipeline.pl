...
GetOptions(
   ...
   "step:s",
   #step1
   "data_list:s","read_len:s","min_alignment_len:s","group:s","fqp:s","adp:s","rf_opts:s","host:s","hvf:s","qc_options:s",
   #step2
   "reads_list:s",
   #step3
...
if($opt{min_alignment_len}){
    $metaphlan .= "--min_alignment_len $opt{min_alignment_len} ";
}
