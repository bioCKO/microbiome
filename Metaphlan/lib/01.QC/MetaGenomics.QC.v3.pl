...
$qsubopt         = " --qopts='-q micro1.q,all.q,micro.q,novo.q -V ' ";
...
if ( $alnP eq "bowtie" && !$alnopt  ) {
    $alnopt ||= " --end-to-end --sensitive -I 200 -X 400 --threads 8 ";
}elsif ( $alnP eq "soap" && !$alnopt  && $read_len == 150 ) {
...
Usage1:perl $0 <Project.info> <out.dir>
  ...
  *-read_len            input reads length, important for soapaligner.defult=150.
  ...
  --method              the alignment method for removing host contamination,defult[bowtie]
  --me_opts             the option value for --method ,bowtie[defult];soap;bwa.
...
if (@hostfa) {
	...
	foreach(@hostfa){
        my $hostfa_name = abs_path($_);
		my $hostname=$1 if($hostfa_name=~ /.*\/(.*)/);
...
if ( $hostfa && -s $hostfa ) {
    ...
    elsif($alnP eq "bowtie"){
        if(-s "$hostfa.1.bt2l"){
            push @IndexDB,"$hostfa";
            $hostfa = abs_path($hostfa); ###modify by zhangghao 20180426
            my $hostname=$1 if($hostfa=~/.*\/(.*)/);
            my $index = $shdir . "/Index/$hostname"; 
            `mkdir -p $index ` if ( !-d $index );
            print STAT "date >$index/Index_data.fin\n\n";
        }else{
            $hostfa = abs_path($hostfa);
            my $hostname=$1 if($hostfa=~/.*\/(.*)/);
            #my $hostname=$1 if($hostfa=~/.*\/(.*)\.(fa|fna|fasta)$/);
            my $index = $shdir . "/Index/$hostname";
            my $suffix=basename($hostfa);
            my $hostfaIndex = $index . "/" . $suffix;
            `mkdir -p $index ` if ( !-d $index );
            print STAT "cd $index\nln -s $hostfa\n$bowtie_build --large-index $hostfa $suffix\ndate >$index/Index_data.fin\n\n";
             push @IndexDB,"$hostfaIndex";
        }
    }
	else {
...
#my $fqs_dir = $SystemClean . "/$key";
		if (@IndexDB) {
			...
			if ( $alnP eq "soap" ) {
                ...
                my($read_a,$read_b);
                foreach my $IndexDB(@IndexDB){
                   #my $hostdir=$1 if($IndexDB=~/.*\/(.*)\.(fa|fna|fasta)$/);
                    my $hostdir=$1 if($IndexDB=~/.*\/(.*\.(fa|fna|fasta)$)/);
                    my $host_sample2=$HostClean."/$hostdir/$key";
                    $read_a="$samploutdir{$key}/$fqname.fq1.gz" if(!$read_a);
                    $read_b="$samploutdir{$key}/$fqname.fq2.gz" if(!$read_b);
                    (-s $host_sample2) || `mkdir -p $host_sample2`;
                    print HOST "$sh_contral $shdir/Index/$hostdir/Index_data.fin\n",
                            "cd $host_sample2\n$bowtie -x $IndexDB -1 $read_a -2 $read_b 2>$host_sample2/$fqname.bowtie.log | $samtools view -Sb - > $fqname.bowtie.bam\n",
#                          "cd $host_sample2\n$bowtie -x $IndexDB -1 $read_a -2 $read_b -S $fqname.bowtie.sam 2>$host_sample2/$fqname.bowtie.log \n",
                          "$extractread $read_a $read_b $fqname.bowtie.bam $host_sample2\n",
                          "$fqcheck -r $host_sample2/$fqname.nohost.fq1.gz -c $host_sample2/$fqname.nohost.fq1.check & \n",
                          "$fqcheck -r $host_sample2/$fqname.nohost.fq2.gz -c $host_sample2/$fqname.nohost.fq2.check \n";
                    push @host_sample,$host_sample2;
                    print HOST "date > $host_sample2/$fqname.finish\n";
                    $check_finish_sample_insertsize .= " $host_sample2/$fqname.finish ";
                    $check_total_finish .= " $host_sample2/$fqname.finish ";
...
$qsub_run .= "$superl_work --prefix DataStat --resource vf=1G --splits \"$splits\" step2.DataStat.sh\n" if($step=~/3/);
...
if ($locate) {    
    print WORK "cd $shdir\n$locate_run\n";
 system"cd $shdir;$locate_run";
}else{
    print WORK "cd $shdir\n$qsub_run\n";
   system"cd $shdir; $qsub_run";
}
