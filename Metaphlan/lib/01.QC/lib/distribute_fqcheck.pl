...
$plot .= "set yrange [-2:70]\n";
$plot .= "set title 'Base percentage composition along reads'\n";
$plot .= "set xlabel 'Position along reads'\n";
$plot .= "set ylabel 'Percent Value'\n";
$plot .= "plot '$out.base' u 1:2 w l lw 1 t 'A',";
$plot .= " '' u 1:3 w l lw 2 t 'C',";
$plot .= " '' u 1:4 w l lw 2 t 'G',";
$plot .= " '' u 1:5 w l lw 2 t 'T',";
$plot .= " '' u 1:6 w l lw 3 t 'N'";
$plot .= ", '' u ($last_cycle[1]+0.5):(50) w impulses lt 6 notitle", if ($last_cycle[2]);
$plot .= "\n";
open TMP, ">$out.base.tmp";
print TMP $plot;
close TMP;
system "$gnuplot $out.base.tmp";
system "/usr/bin/convert ps:$out.base.ps $out.base.png";
...
$plot .= "set title 'Quality Distribution of Base.'\n";
$plot .= "set xlabel 'Position along reads'\n";
$plot .= "set ylabel 'Quality Value'\n";
$plot .= "set key off\n";
$plot .= "set style fill solid noborder\n";
$plot .= "plot '$out.qual' u (\$1):(\$2):(\$3*0.0005):(0.3) w boxxy lt 2;\n";
open TMP, ">$out.qual.tmp";
print TMP $plot;
close TMP;
system "$gnuplot $out.qual.tmp";
system "/usr/bin/convert ps:$out.qual.ps $out.qual.png";
...
sub set_plot {
	...
	if ($version =~ /gnuplot 4\.0/) {
		...
		$set_plot = "set terminal postscript portrait color size 10.4, 7.8\n";
	}
	$set_plot .= "set grid\n";
	return $set_plot;
}
