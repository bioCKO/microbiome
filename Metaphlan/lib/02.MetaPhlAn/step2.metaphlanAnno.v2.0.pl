...
#set options
my($data_list, $vf, $num_proc, $shdir, $outdir,$notrun, $locate, );
...
use PATHWAY;
(-s "$Bin/../../bin/Pathway_cfg.ori.txt") || die"error: can't find config at $Bin/../../bin, $!\n";
my ($super_worker, $bowtie2db_mpa) = get_pathway("$Bin/../../bin/Pathway_cfg.ori.txt", [qw(SUPER_WORK BOWTIE2DB_MPA)]);
my $fq2fa = "$Bin/lib_ori/fq2fa.v2.pl";
my $metaphlan="$Bin/lib_ori/metaphlan.py";
...
if($data_list && -s $data_list || die "$help"){
    ...
    while(<F>){
        ...
        print SH 
    "rm $outdir/01.bowtie/$sampleid.bt2out
$Bin/lib_ori/metaphlan.py --bowtie2db $bowtie2db_mpa --bt2_ps sensitive --nproc $num_proc --input_type multifasta --bowtie2out $outdir/01.bowtie/$sampleid.bt2out $outdir/00.prepare.bzip2/$sampleid.fa > $outdir/02.profiled_samples/$sampleid.txt\n\n";  
...
$Bin/lib_ori/utils/merge_metaphlan_tables.py $outdir/02.profiled_samples/*.txt > $outdir/03.Stat/merged_abundance_table.txt
$Bin/lib_ori/plotting_scripts/metaphlan_hclust_heatmap.py -c bbcry --minv 0.1 -s log --in $outdir/03.Stat/merged_abundance_table.txt --out $outdir/03.Stat/abundance_heatmap.png
#relative abundance
perl $Bin/lib_ori/abundance.trans.v2.pl $outdir/03.Stat/merged_abundance_table.txt $outdir/03.Stat/Relative\n";
    close SH;
