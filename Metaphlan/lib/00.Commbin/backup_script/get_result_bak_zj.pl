...
use Data::Dumper;
# set default options
my %opt = (indir=>"",result=>".",shdir=>"./Shell",cpu=>'1',avf=>'500M',qopts=>' --qopts \' -q all.q,micro.q \' ',type=>'soapdenovo',contact=>'yuanyuqi',);
GetOptions (\%opt,"indir:s","result:s","shdir:s","rawdata:s","cpu:i","avf:s","help:s","locate","s2","notrun","host:s","ipath","vs","type:s","contact:s","info:s","lefse"); 
...
Options:
      ...
      *-type      [str]      choose different assembly Methods discription according to deffirent type,default=soapdenovo.
      ...
      --contact   [str]       contact name, default=yuanyuqi
...
#		($opt{s2}) || system "ln -s $cover/Unigenes.coverage.single.even.tree $total[0]";
        ...
        if(-s "$genedir/GeneStat/venn_fplower/venndata"){system "ln -s $genedir/GeneStat/venn_flower/venndata $stat[3]";}
        if(-s "$genedir/GeneStat/venn_flower/flowerdata"){system "ln -s $genedir/GeneStat/venn_flower/flowerdata $stat[3]";}
...
if(-s "$indir/04.TaxAnnotation"){ 
    (-d $dir[3]) || mkdir($dir[3]);
    my @TaxAnno;
    $opt{s2}?(@TaxAnno=qw(MAT MicroNR_Anno top)):(@TaxAnno=qw(MAT MicroNR_Anno Cluster_Tree heatmap PCA top NMDS));
	for (@TaxAnno){
...
#NMDS
    if (-s "$indir/04.TaxAnnotation/MicroNR/MicroNR_stat/NMDS" && ! $opt{s2}){
        for(qw(class family genus order phylum species)){
            ...
            system "ln -s $indir/04.TaxAnnotation/MicroNR/MicroNR_stat/NMDS/$_/{NMDS_2.pdf,NMDS_2.png,NMDS.pdf,NMDS.png,NMDS_scores.txt} $dir[3]/NMDS/$_ ";
        }
    }
...
#CAZy
	if (-s "$indir/05.FunctionAnnotation/CAZy"){  
	    ...
	    my @CazyAnno;
    	$opt{s2}?(@CazyAnno=qw(CAZy_MAT CAZy_Anno)):(@CazyAnno=qw(CAZy_MAT CAZy_Anno heatmap PCA NMDS));
	    foreach(@CazyAnno){
         ...
         #NMDS
            if(!$opt{s2} && -s "$indir/05.FunctionAnnotation/CAZy/CAZy_stat/NMDS"){
                for (qw(ec level1 level2)){
                    ...
                    system "ln -s $indir/05.FunctionAnnotation/CAZy/CAZy_stat/NMDS/$_/{NMDS_2.pdf,NMDS_2.png,NMDS.pdf,NMDS.png,NMDS_scores.txt} $dir[4]/CAZy/NMDS/$_";
                }
            }
          ...
          #NMDS
            if(!$opt{s2} && -s "$estat/NMDS"){
                for(qw(level1 level2 og)){
                    ...
                    system "ln -s $estat/NMDS/$_/{NMDS_2.pdf,NMDS_2.png,NMDS.pdf,NMDS.png,NMDS_scores.txt} $dir[4]/eggNOG/NMDS/$_";
                }
            }
...
#		($opt{s2}) && system"rm -f $dir[4]/KEGG/KEGG_Anno/Unique.Genes.level1.bar.tree.*";
        ...
        if(!$opt{s2} && -s "$kstat/NMDS"){
            for (qw(ec ko level1 level2 level3)){
                ...
                system "ln -s $kstat/NMDS/$_/{NMDS_2.pdf,NMDS_2.png,NMDS.pdf,NMDS.png,NMDS_scores.txt} $dir[4]/KEGG/NMDS/$_";
            }
        }
...
my $cds_file="$dir[2]/GenePredict/total.CDS.stat.xls";#
