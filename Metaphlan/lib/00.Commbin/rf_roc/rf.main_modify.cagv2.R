...
if(! is.null(opt$headvar))
{
	...
	var<-system(paste("awk -F \"\t\" '{print NF}' ",infile," | head -n 1"),intern=T)
...
library(ggplot2)
set.seed(999)
source("/PUBLIC/software/MICRO/share/MetaGenome_pipeline/MetaGenome_pipeline_V5.0/lib/00.Commbin/rf_roc/randomforest.crossvalidation.last.r")
...
#####draw cv_error_plot.pdf
	...
	outpng<-paste(outdir,"/","cverrof.png",sep="")
	nvar<-result[[1]]$n.var[-length(result[[1]]$n.var)]
	mean_cverror<-rowMeans(error.cv)[-length(result[[1]]$n.var)]
# cairo_pdf(outpdf)   
#    pdf(outpdf)
    cv_plot<-ggplot()
    cv_plot<-cv_plot+geom_point(aes(nvar,mean_cverror))+geom_path(aes(x=nvar,y=mean_cverror))
...
######        
    ...
    #draw imp plot
outpdf=paste(outdir,"/","impplot_MeanDecreaseAccuracy",head_im_var,".pdf",sep="");
outpng=paste(outdir,"/","impplot_MeanDecreaseAccuracy",head_im_var,".png",sep="");
...
#   impplot
    impplot<-ggplot()
    impplot<-impplot+geom_bar(aes(x=reorder(rownames(imp),imp[,1]),y=imp[,1]),stat="identity",fill=brewer.pal(12,"Set3")[1],color="black")+coord_flip()
    impplot<-impplot+theme(panel.background = element_blank()
                                   ,panel.border = element_rect(colour = "black",fill = NA),axis.text = element_text(face = "bold",colour = "black"))
    impplot<-impplot+labs(x=" ",y="MeanDecreaseAccuracy")+scale_size_continuous(name="")
...
system(paste("convert -density 200 ",outpdf,outpng),intern=F)
#####Gini
outpng=paste(outdir,"/","impplot_MeanDecreaseGin",head_im_var,".png",sep="");
outpdf=paste(outdir,"/","impplot_MeanDecreaseGin",head_im_var,".pdf",sep="");
    impplot<-ggplot()
    impplot<-impplot+geom_bar(aes(x=reorder(rownames(imp),imp[,2]),y=imp[,2]),stat="identity",fill=brewer.pal(12,"Set3")[2],color="black")+coord_flip()
    impplot<-impplot+theme(panel.background = element_blank(),panel.border = element_rect(colour = "black",fill = NA),axis.text = element_text(face = "bold",colour = "black"))
    impplot<-impplot+labs(x=" ",y="MeanDecreaseGin")+scale_size_continuous(name="")
    impplot 
ggsave(outpdf)
system(paste("convert -density 200 ",outpdf,outpng),intern=F)
#    ggsave(paste("impplot.",head_im_var,"pdf",sep=""),path=outdir)
	...
	outpng = paste(outdir,"/","top_",head_im_var,"testset.var_predict_box_plot.png",sep="")
	pdf(outpdf)
	...
	if( length(unique(as.character(Y))) ==2){ #只有两个分组时候才画ROC
		...
		outpng = paste(outdir,"/","testset.ROC.png",sep="")
		pdf(outpdf)
	...
	outpng = paste(outdir,"/","top_",head_im_var,"trainset.var_predict_box_plot.png",sep="")
	pdf(outpdf)
	...
	if( length(unique(as.character(Y))) ==2){ #只有两个分组时候才画ROC
		...
		outpng = paste(outdir,"/","trainset.ROC.png",sep="")
...
##### draw top10 impplot 
outpdf = paste(ori_outdir,"/","imptop10MD.pdf",sep="")
outpng = paste(ori_outdir,"/","imptop10MD.png",sep="")

	x10=head(rownames(imp)[order(imp[,1],decreasing = T)],n=10L)
	y10=imp[,1][head(order(imp[,1],decreasing = T),n=10L)]
	x10=reorder(x10,y10)
	#x10

	impplot<-ggplot()
	impplot<-impplot+geom_bar(aes(x=x10,y=y10),stat="identity",fill=brewer.pal(12,"Set3")[1],color="black")+coord_flip()
	impplot
	impplot<-impplot+theme(panel.background = element_blank()
			                       ,panel.border = element_rect(colour = "black",fill = NA),axis.text = element_text(face = "bold",colour = "black"))
	impplot<-impplot+labs(x=" ",y="MeanDecreaseAccuracy")+scale_size_continuous(name="")
    ...
    system(paste("convert -density 200 ",outpdf,outpng),intern=F)
	 ...
	 outpdf = paste(ori_outdir,"/","imptop10MG.pdf",sep="")
	  outpng = paste(ori_outdir,"/","imptop10MG.png",sep="")
	x10=head(rownames(imp)[order(imp[,2],decreasing = T)],n=10L)
	y10=imp[,2][head(order(imp[,2],decreasing = T),n=10L)]
	x10=reorder(x10,y10)
	impplot<-ggplot()
	impplot<-impplot+geom_bar(aes(x=x10,y=y10),stat="identity",fill=brewer.pal(12,"Set3")[2],color="black")+coord_flip()
	impplot
	impplot<-impplot+theme(panel.background = element_blank()
			                               ,panel.border = element_rect(colour = "black",fill = NA),axis.text = element_text(face = "bold",colour = "black"))
	impplot<-impplot+labs(x=" ",y="MeanDecreaseGini")+scale_size_continuous(name="")
	ggsave(outpdf)
	system(paste("convert -density 200 ",outpdf,outpng),intern=F)
...
outpng = paste(ori_outdir,"/","testset.point_auc.png",sep="")
...
auc_plot<-auc_plot+theme(panel.background = element_blank(),panel.border = element_rect(colour = "black",fill = NA)
                         ,axis.text = element_text(face="bold",colour = "black"))+labs(x="the numbers of variable",y="AUC")
auc_plot
#   dev.off()
ggsave(outpdf)
system(paste("convert -density 200 ",outpdf,outpng),intern=F)
...
outpng<-paste(ori_outdir,"/","testset_auc.png",sep="")
...
for (i in 1:length(roc_test_list))
{
  if (i == 1)
  {
  plot.roc(roc_test_list[[i]]$roc,col=col$V1[i])
 #   plot.roc(roc_test_list[[i]]$roc,col=brewer.pal(12,"Set3")[i])
    leg[i]<-paste(roc_test_list[[i]]$num,":","AUC=",round(roc_test_list[[i]]$roc$ci[2],2),"%")
   leg_col[i]<-col$V1[i]
#    leg_col[i]<-brewer.pal(12,"Set3")[i]
  }
  else{
 plot.roc(roc_test_list[[i]]$roc,add=T,col=col$V1[i])
#  plot.roc(roc_test_list[[i]]$roc,add=T,col=brewer.pal(12,"Set3")[i])
  leg[i]<-paste(roc_test_list[[i]]$num,":","AUC=",round(roc_test_list[[i]]$roc$ci[2],2),"%")
  leg_col[i]<-col$V1[i]}
#  leg_col[i]<-brewer.pal(12,"Set3")[i]}
}
...
outpng = paste(ori_outdir,"/","trainset.point_auc.png",sep="")
...
outpng<-paste(ori_outdir,"/","trainset_auc.png",sep="")
...
for (i in 1:length(roc_train_list))
{
  if (i == 1)
  {
  plot.roc(roc_train_list[[i]]$roc,col=col$V1[i])
#    plot.roc(roc_train_list[[i]]$roc,col=brewer.pal(12,"Set3")[i])
    leg[i]<-paste(roc_train_list[[i]]$num,":","AUC=",round(roc_train_list[[i]]$roc$ci[2],2),"%")
    leg_col[i]<-col$V1[i]
#    leg_col[i]<-brewer.pal(12,"Set3")[i]
  }
  else{
 plot.roc(roc_train_list[[i]]$roc,add=T,col=col$V1[i])
#  plot.roc(roc_train_list[[i]]$roc,add=T,col=brewer.pal(12,"Set3")[i])
  leg[i]<-paste(roc_train_list[[i]]$num,":","AUC=",round(roc_train_list[[i]]$roc$ci[2],2),"%")
 leg_col[i]<-col$V1[i]
#  leg_col[i]<-brewer.pal(12,"Set3")[i]
  }
