...
################## NMDS withcluster #############
        cairo_pdf(filename="$outdir/NMDS_withcluster.pdf",height=10,width=12)
        par(mar=c(4,4,4,10),mgp=c(2,0.5,0),bg="white",cex.lab=1.2)
        plot( scores, type="n", xlab="MDS1", ylab="MDS2", main="NMDS plot")
        abline(h=0,v=0,lty=2)
        s.class(scores, fac=as.factor(plotdata\$group), grid=F, xax = 1, yax = 2,cellipse=0.8,clabel=0,cpoint=1.5,add.plot=TRUE,col=col,pch=19)
        text(plotdata\$MDS1,plotdata\$MDS2,labels=as.vector(plotdata\$sample),cex=1.2,pos=3,offset=0.5)
        text(max(plotdata\$MDS1),max(plotdata\$MDS2),label="$stress",adj = c(1,0.5))
        legend("right",inset=c(-0.20,0),xpd=TRUE,bty="n",legend=as.vector(levels(plotdata\$group)),col=col,pch=19,cex=1.5,pt.cex=1.6)

dev.off()
     cairo_pdf(filename="$outdir/NMDS_withcluster_2.pdf",height=10,width=12)
...
system "$R -f $outdir/plot_NMDS.R
		...
		$convert -density 300 $outdir/NMDS_withcluster.pdf $outdir/NMDS_withcluster.png
		$convert -density 300 $outdir/NMDS_withcluster_2.pdf $outdir/NMDS_withcluster_2.png
";
