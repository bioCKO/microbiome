...
my ($super_worker,$bowtie2,$bowtie_build,$samtools) = get_pathway("$Bin/../../../bin/Pathway_cfg.txt",[qw(SUPER_WORK Bowtie Bowtie_bulid SAM )],$Bin,$lib);
...
my $qopts_soap= "\' -q micro1.q,micro2.q  -V \'"; ##all.q 2 pub.q by zhanghao 20180225
    ...
    while (<OR>) {
        ...
        if($opt{index_f} && -s $opt{index_f}){
         ...
         print LISTUM "$sample\t$opt{outdir}/$sample/$basename.unmapping.fq1.gz,$opt{outdir}/$sample/$basename.unmapping.fq2.gz\n";
            ...
            if($bowtie2{$sample}){
             print SH "cd $opt{outdir}/$sample\n";
             print SH "$bowtie2{$sample} -1 $clean_reads[0] -2 $clean_reads[1] | $samtools view -Sb - > $basename.bowtie.bam 2> bowtie.log\n";
            }else{
                warn "step1:the index_f for SAMPLE:$sample can not find, please check index_f\n";}
            print SH "$samExtra $clean_reads[0] $clean_reads[1] $basename.bowtie.bam $basename.unmapping\n" if ($opt{unmap});
            $opt{genel} ?
            print SH
            "$gene_profiling --sam $basename.bowtie.bam  --out $sample.readsNum.xls\n\n" :
            print SH "\n";
           print LIST "$sample\t$opt{outdir}/$sample/$sample.readsNum.xls\n";
        print LISTUM "$sample\t$opt{outdir}/$sample/$basename.unmapping.fq1.gz,$opt{outdir}/$sample/$basename.unmapping.fq2.gz\n";
        }
        ...
        print SH "cd $opt{outdir}/$sample\n";
        print SH "$bowtie2 -1 $clean_reads[0] -2 $clean_reads[1] | $samtools view -Sb - > $basename.bowtie.bam  2> bowtie.log\n";
        print SH "$samExtra $clean_reads[0] $clean_reads[1] $basename.bowtie.bam $basename.unmapping\n" if ($opt{unmap});
        $opt{genel} ?
            print SH
            "$gene_profiling --sam $basename.bowtie.bam --out $sample.readsNum.xls\n\n":
        ...
        print LISTUM "$sample\t$opt{outdir}/$sample/$basename.unmapping.fq1.gz,$opt{outdir}/$sample/$basename.unmapping.fq2.gz\n";
         ...
         if ( $opt{mapmethod} eq "soap") { $sample2coverage{$sample} .= "$basename.PE.soap $basename.SE.soap ";}elsif($opt{mapmethod} eq "bowtie"){$sample2coverage{$sample} = "$basename.bowtie.bam ";}
    }
    close SH;
    close OR;
    close LIST;
...
if ($opt{step}=~/2/ ) {
    ...
    foreach my $sample (keys %sample2coverage){
	    ...
	    #"$samtools sort $sample.bowtie.bam -o $sample.bowtie.sorted.bam\n",
                "$samtools sort $sample2coverage{$sample} -o $sample.bowtie.sorted.bam\n",
    ...
    if ( $opt{mapmethod} eq "soap"){ 
    $qsub_run .= "$super_worker_soap step2.soap.coverage.sh -splits '$splits' --prefix coverage  --dvf 10G\n";
    }elsif($opt{mapmethod} eq "bowtie"){
        $qsub_run .= "$super_worker_bowtie step2.soap.coverage.sh -splits '$splits' --prefix coverage --dvf 10G\n";
    }
}
...
if ($opt{step}=~/4/ &&  $opt{index} && !$opt{index_f}) {
    ...
    $qsub_run .= "$super_worker_table --resource 16G step4.coverage_table.sh  -splits '$splits' --dvf 20G --prefix coverage_table --qopts ' -q micro1.q,micro2.q -V '\n";
}
}
