...
sub readsam(){
    my ( $sam_file, $gene ) = @_;
    if ( -s $sam_file ) {
               my $oo=` tail -2 $sam_file\|awk 'NR==1{print \$1}'`;
               my $ot=` tail -2 $sam_file\|awk 'NR==1{print \$3}'`;
               my $to=` tail -2 $sam_file\|awk 'NR==2{print \$1}'`;
               my $tt=` tail -2 $sam_file\|awk 'NR==2{print \$3}'`;
               chomp $oo;
               chomp$ot;
               chomp$to;
               chomp$tt;
