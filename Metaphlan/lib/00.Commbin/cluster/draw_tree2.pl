...
GetOptions(
    ...
    "color1:s","color2:s","color3:s","turn:s","root","scal_title:s","top","one","color:s","addwidth:f",
    "barlen:i","btitle:s","bcolor:s","trantab","flank_b:i","bline","bun:s","bh:f","group:s","dotcol:s",
    ...
    "rect:s","addheight:f","symwidth:f","symheight:f","symsize:f","rectsort","symtextnum:i","gtitle:s",
    "Bdot","Binner","match","msplit:s","tsize:i"
);
...
my @bcolor = $opt{bcolor} ? split/,/,$opt{bcolor} :
    qw(crimson royalblue lightseagreen orange mediumpurple palegreen lightcoral dodgerblue lawngreen olive 
    yellow fuchsia  mediumslateblue darkviolet purple sienna tan chocolate skyblue turquoise cadetblue deeppink darkorchid);#lightcoral - salmon
if(-s $rgb_txt){
    my %uniq_color = map {($_,1)} @bcolor;
    for(`awk '{print \$1}' $rgb_txt`){
        chomp;
        !$uniq_color{$_} && ($_ ne 'white') && (push @bcolor,$_);
    }
}
my %recth;
my @rect_sym;
get_rect($opt{rect},\%recth,\@rect_sym,\@bcolor,$opt{rectsort});

my %colorh;
my (@colors, @gline,@glcolor);
   ...
   %colorh = split/\s+/,`less $opt{color}`;
    ...
    for(`less $opt{group}`){
        ...
        if(!$uniq_group{$l[1]}){
            ...
            push @glcolor,$bcolor[$#gline];
        }
        $colorh{$l[0]} = $uniq_group{$l[1]};
    }
}
if($opt{gtitle}){
    unshift @gline,"}$opt{gtitle}:";
    unshift @glcolor,0;
}
...
my ($root_len,$root_B,$no_blen) = get_nhx($nhx,\@spe,\@tree,\@branch,\%spe_num,\%node_yh,\%edge,\%colorh,
    \@colors,\@end_branch,$opt{spematch},$opt{match}, $opt{msplit}, $opt{species} ? \%species_sel : 0);#sub1
...
my $max_height = $opt{scal_title} ? 3.8*$fsize : 2.8*$fsize;
($max_height < $opt{flank_y}) && ($max_height = $opt{flank_y});
my $max_len = $opt{flank_x} + $opt{width};
my $pwidth = $opt{width} + $opt{flank_x} + $max_width + ($opt{addwidth}||0);
my $pheight = $opt{height} + $opt{flank_y} + $max_height + ($opt{addheight}||0);
...
my %dotcol;
if($opt{dotcol}){
    %dotcol = (-s $opt{dotcol}) ? split/\s+/,`less $opt{dotcol}` : split/,/,$opt{dotcol};
}
...
if($opt{type} == 4){
    ...
    draw_cycle_tree($svg,$cx,$cy,$cr2,$cr1,$opt{root},$root_len,$root_B,$x_unit,$y_unit,\@tree,\@spe,\%node_xw,\%node_yh,$opt{fsize},
        $opt{lw},$opt{end},$opt{color1},$opt{color2},$opt{color3},\@colors,\%edge,$opt{tran},$opt{opacity},$opt{dot},$opt{noname},\%dotcol);
    if($blen){
    ...
    draw_tree($svg,$opt{flank_x},$opt{flank_y},$opt{root},$root_len,$root_B,$x_unit,$y_unit,\@tree,\@spe,\%node_xw,
        ...
        $opt{inner_fill},$opt{opacity},$opt{noname},$opt{dot},\%dotcol,\%recth);
    my ($scal_x,$scal_y) = ($opt{flank_x}, $opt{top} ? 0 : $opt{flank_y}+$opt{height}+$fsize/3);
    $no_blen ||
    draw_scal($svg,$max_branch,$x_unit,$opt{width},$scal_x,$scal_y,$fsize,$opt{lw},$opt{scal_title},$opt{one},$opt{tsize});#sub3
    if(@rect_sym){
        my (@rectsyms,@rectcolor);
        my ($rectx,$recty) = ($opt{flank_x}+$opt{width}+$opt{fsize}, $opt{flank_y}+$opt{height}+$opt{fsize}/2);
        for my $i(0..$#rect_sym){
            my $rect_group = $svg->group("transform","rotate(45,$rectx,$recty)",'stroke','none', 'fill','black','font-family','Arial');
            $rect_group->text('x',$rectx,'y',$recty+0.8*$opt{fsize}/3,'-cdata',$rect_sym[$i]->[0], 'font-size', 0.8*$opt{fsize}, 'text-anchor','star');
            $rectx += $opt{fsize};
            push @rectsyms,"}$rect_sym[$i]->[0]->[0]:";
            push @rectcolor,0;
            for my $j(1..$#{$rect_sym[$i]}){
                push @rectsyms,"]$rect_sym[$i]->[$j]->[0]";
                push @rectcolor,$rect_sym[$i]->[$j]->[1];
            }
        }
        unshift @gline,@rectsyms;
        unshift @glcolor,@rectcolor;
    }
    if($blen){
        draw_bar($svg,\@spe,\%bardata,$y_unit,$bar_star,$opt{flank_y},$opt{barlen},$rate,$fsize,$blen,
        \@bcolor,$opt{bline},$opt{bun},$opt{btitle});
        draw_sym($svg,\@bsym,$bar_star+$opt{barlen},$opt{flank_y}+(1-$rate)*$y_unit/2,$opt{flank_b},$fsize,\@bcolor);
    }
    @gline && draw_sym($svg,\@gline,$fsize,$opt{flank_y},$opt{symwidth}||$opt{flank_x},$fsize,\@glcolor,2,$opt{symheight},
            $opt{symsize},$opt{symtextnum});
}
print $svg->xmlify;

#===================================================================
sub get_rect{
    my ($rect,$recth,$rect_sym,$color,$rectsort) = @_;
    ($rect && -s $rect) || return(0);
    open REC,$rect || die$!;
    chomp(my @head = split/\s+/,<REC>);
    my @num = (0) x $#head;
    shift @head;
    my @symcolor;
    while(<REC>){
        ...
        my $spe_name = shift @l;
        for my $i(0..$#l){
            if(!($symcolor[$i] && defined $symcolor[$i]->{$l[$i]})){
                $symcolor[$i]->{$l[$i]} = $color->[$num[$i]||=0];
                push @{$rect_sym->[$i]},[$l[$i],$color->[$num[$i]]];
                $num[$i]++;
            }
            push @rcolor,$symcolor[$i]->{$l[$i]};
        }
        $recth->{$spe_name} = [@rcolor];
    }
    close REC;
    if($rectsort){
        for my $i(0..$#head){
            my @rsym = sort {$a->[0] cmp $b->[0]} @{$rect_sym->[$i]};
            my %colchange;
            for my $j(0 .. $#rsym){
                $colchange{$rsym[$j]->[1]} = $color->[$j];
                $rsym[$j]->[1] = $color->[$j];
            }
            @{$rect_sym->[$i]} = @rsym;
            for my $v(values %{$recth}){
                $v->[$i] = $colchange{$v->[$i]};
            }
        }
    }
    for my $i(0..$#head){
        unshift @{$rect_sym->[$i]},[$head[$i]];
    }
}
sub draw_cycl_bar{
...
sub read_bardata{
    my ($barstat,$bsym,$bardata,$tran,$bsplit,$barlens,$branks,$nohead,$lgranks,$scal_arr) = @_;
    ...
    if($lgranks && @$lgranks){
        for my $i(@$lgranks){
            my $min = int(log($minbarlen[$i]));
            (log($minbarlen[$i]) < $min) && ($min--);
            my $max = int(log($barlens->[$i]));
            (log($barlens->[$i]) > $max) && ($max++);
            $scal_arr && ($scal_arr->[$i] = [$min,$max]);
            $max -= $min;
            $barlens->[$i] = 1;
            for my $v(values %{$bardata}){
                $v->[$i] = (log($v->[$i])-$min)/$max;
...
sub draw_sym{
    my ($svg,$bsym,$flank_x,$flank_y,$bwidth,$fsize,$color,$cycle,$symheight,$symsize,$symtexnum) = @_;
    my $nlen = 0;
    $cycle ||= 0;
    for (@$bsym){
        my $temp_len = length;
    ...
    if($symheight){
        $may_size = $symheight/@$bsym/1.5;
        ($may_size < $fsize) && ($fsize = $may_size);
    }
    $symsize && ($fsize = $symsize);
    my $sx = $flank_x + $fsize;
    my $sy = $flank_y;
    my $tex_num = 0;
    $nlen = 0;
    for my $i(0..$#$bsym){
        ...
        if($bsym->[$i] =~ s/^\}//){
            $ssx = $sx;
            $tex_num++;
            if($symtexnum && $tex_num == $symtexnum){
                $sx += $nlen+$fsize;
                $ssx += $nlen+$fsize;
                $tex_num = 1;
                $sy = $flank_y;
...
sub get_nhx{
#===========
    my ($nhx,$spe,$tree,$branch,$spe_num,$node_yh,$edge,$colorh,$colors,$end_branch,$spematch,$match,$msplit,$spe_sel) = @_;
    $nhx =~ s/\'[\.\d]+?:/:/g;
    $nhx =~ s/;|\s|\&\&NHX:|'//g;
    my $i = "0000001";
    my $j = 0;
    ...
    for(@$spe){
        ...
        push @{$edge->{$i}},$j;
        if($match && %$colorh){
            for my $k(keys %$colorh){
                m/$k/ || next;
                $colors->[$j] = $colorh->{$k};
                last;
            }
        }elsif($msplit && %$colorh){
            my $k2 = (split/$msplit/)[0];
            $colors->[$j] = $colorh->{$k2};
        }else{
...
sub draw_cycle_tree{
    my ($svg,$cx,$cy,$r1,$r2,$root,$root_len,$root_B,$x_unit,$y_unit,$tree,$spe,$node_xw,$node_yh,$fsize,
        $lw,$to_end,$color1,$color2,$color3,$colors,$edge,$tran,$opacity,$dot,$noname,$dotcol) = @_;
    ($tree && @$tree) || return(0);
    $tran ||= 0;
    my $add = 0;
    $lw ||= 1;
        ...
        for my $j(0..$#{$tree->[$i]}){
            ...
            for my $branch(@{$tree->[$i]->[$j]}){
                ...
                my ($r1,$a1) = arc_path($svg,'d',[$cx,$cy,$r0,$a0,$tran+$node_yh->{$branch->[0]}*$y_unit,$add,
                    $L,$L2,$lw,$Bvalue,$fsize,$color3,$spe_name,$line_color,$spe_color,$opacity,$dot,$noname,$dotcol],'stroke',$line_color,
...
sub arc_path{
    my $svg = shift;
    my %path = @_;
    my ($cx,$cy,$r,$a1,$a2,$add,$L,$L2,$lw,$Bvalue,$fsize,$color3,$name,$color1,$color2,$opacity,$dot,$noname,$dotcol) = @{$path{d}};
    ...
    $svg->path(%path);
    $noname || path_name($svg,$name,$a2,$xy[-2],$xy[-1],$fsize,$color2,$dot,0,$dotcol);
    path_name($svg,$Bvalue,$a1,$xy[0],$xy[1],$fsize,$color3,$opt{Bdot},$opt{Binner} ? 'end' : 0);
    $L && $opacity && annulus_path($svg,'d',[$cx,$cy,$r,$r+$L,$a1,$a2,$add],'fill-opacity', $opacity,'stroke','none','fill',$color1);
    ($r+$L,$a2-$add);
}
sub path_name{
    my ($svg,$name,$a2,$cx,$cy,$fsize,$color,$dot,$text_anchor,$dotcol) = @_;
    $name || return(0);
    $color ||= 'black';
    $text_anchor ||= 'start';
    my $rra = 360*$a2 - 90;
    ...
    simpnum($cx,$cy,2);
    ($dot || ($dotcol && $dotcol->{$name})) && $svg->circle('cx',$cx,'cy',$cy,'r',$fsize/3,"fill",($dotcol->{$name}||$color),"stroke",'none');
...
sub draw_tree{
    my ($svg,$flank_x,$flank_y,$root,$root_len,$root_B,$x_unit,$y_unit,$tree,$spe,$node_xw,$node_yh,$fsize,
        $max_len,$lw,$to_end,$type,$color1,$color2,$color3,$colors,$edge,$inner_fill,$opacity,$noname,$dot,
        $dotcol,$recth) = @_;
    ($tree && @$tree) || return(0);
    $flank_y += $y_unit/2;
    ...
    for my $i(reverse (0..$#$tree)){
        for my $j(0..$#{$tree->[$i]}){
            ...
            for my $branch(@{$tree->[$i]->[$j]}){
                ...
                $max_len,$lw,$to_end,$type,$color1,$color2,$color3,$colors,$edge,$inner_fill,$opacity,$noname,
                $dot,$dotcol,$recth);#sub2.2
            }
        }
    }
}
...
#        $svg->line('x1',$root_x,'y1',$by,'x2',$bx,'y2',$by,'stroke',$color1,'stroke-width',$lw);
    ...
    if($branch->[0]=~/^(\d+)$/ && $spe->[$1-1]){
        ...
        ($dot || $dotcol->{$spe_name} ) && $svg->circle('cx',$bx,'cy',$by,'r',$fsize/3,"fill",$dotcol->{$spe_name}||$color2,"stroke",'none');
        if($recth && $recth->{$spe_name}){
            $bx += $fsize/2;
            my $rect_w = $fsize;
            for my $p(@{$recth->{$spe_name}}){
                $svg->rect('x',$bx,'y',$by-$y_unit/2,'width',$rect_w,'height',$y_unit,'stroke','black','fill',$p);#'fill-opacity',0.8);
                $bx += $rect_w;
            }
        }
        $noname ||
...
sub draw_scal{
#=============#
    my ($svg,$max_branch,$x_unit,$width,$scal_x,$scal_y,$fsize,$lw,$title,$one,$ptsize) = @_;
