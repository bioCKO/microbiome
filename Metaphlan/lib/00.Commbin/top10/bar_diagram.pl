...
my ($height,$width, $flank_x,$flank_y, $size_x,$micro_scale,
    ...
    $preci,$help,$h,$barw,$y_mun,$h_pos,$size_ht,$row,$namemark,
...
while (<IN>){
    ...
    ($l[-1] =~ /^\d/) || next;
	$tol_amin++;
	($nohead || $single) && (push @x_group,(shift @l));
    push @ys_group,[@l];
    if($log){
...
if($color){
    if(-s $color){
        unshift @colors,split/\s+/,`awk '{print \$1}' $color`;
    }else{
...
foreach my $i (0 .. $tol_amin - 1){
    ...
    my $ggcolor = ($namemark && $x_group[$i]=~/\*$/) ? 'red' : 'black';
    my $group = $rotate ? $svg->group("transform","rotate($rotate,$t_x,$t_y2)",'stroke','none', 'fill',$ggcolor,'font-size',
        $size_x,'font-family','Arial','text-anchor', $rotate > 0 ? 'start' : 'end') :
    $svg->group('stroke','none', 'fill',$ggcolor,'font-size',$size_x,'font-family','Arial','text-anchor', $texanchor);
