...
open OUT1,">$outdir/$outfile.pic.txt" || die$!;
...
foreach my $key ( keys %top_abundant ) {
    ...
    my $taxonlen=length($full_taxon{$key});
    my $cutname;
    if ($taxonlen>70) {
    $cutname=substr($full_taxon{$key},0,70). "(\.\.\.)";
    }else{
        $cutname=$full_taxon{$key};
    }
    print OUT1 $cutname. "\t" . $line{$key} . "\n";
...
#============================================================================
#R scripts
my $pdf_w =$sample_num*50/155+14;
my $pdf_h =$abundant_limt*13/35+5;
...
if($group){
    if(!$rank_rank || $outfile=~/cluster\.p/){
        ...
        setwd(\"$outdir\")
        x<-read.table(\"$outfile.pic.txt\",sep=\"\\t\",header=T,row.names=1)
        ...
        pheatmap(x,scale=\"row\",annotation_col = annotation_col,filename=\"$outfile.pdf\",height=ceiling($pdf_h),width=ceiling($pdf_w),fontsize=20,cluster_cols=FALSE,annotation_colors=ann_colors,cellwidth=30,cellheight=30)    
        dev.off()";
    }else{ 
        print R "library(pheatmap)
        setwd(\"$outdir\")
        ...
        pheatmap(x,scale=\"row\",annotation_col = annotation_col, annotation_row = annotation_row,filename=\"$outfile.pdf\",height=ceiling($pdf_h),width=ceiling($pdf_w),fontsize=20,cluster_cols=FALSE,annotation_colors=ann_colors,cellwidth=30,cellheight=30)
        dev.off()";
    }
}else{
    if(!$rank_rank || $outfile=~/cluster\.p/){
        ...
        setwd(\"$outdir\")
        x<-read.table(\"$outfile.pic.txt\",sep=\"\\t\",header=T,row.names=1)
        pheatmap(x,scale=\"row\",filename=\"$outfile.pdf\",height=ceiling($pdf_h),width=ceiling($pdf_w),fontsize=20,cluster_cols=FALSE,cellwidth=30,cellheight=30)
        dev.off()";
    }else{
        print R "library(pheatmap)
        setwd(\"$outdir\")
        ...
        pheatmap(x,scale=\"row\",annotation_row = annotation_row,filename=\"$outfile.pdf\",,height=ceiling($pdf_h),width=ceiling($pdf_w),fontsize=20,cluster_cols=FALSE,annotation_colors=ann_colors,cellwidth=30,cellheight=30)
        dev.off()";
    }
}
close R;
...
#(-s "$outdir/$outfile.pic.txt")&&(`rm -r $outdir/$outfile.pic.txt`);
