...
GetOptions (\%opt,"indir:s","result:s","shdir:s","rawdata:s","cpu:i","avf:s","help:s","locate","s2","notrun","host:s","ipath","vs","type:s","contact:s","info:s","lefse","step:s"); 
...
print RE "$release_rm --resultdir $result --info $result/../$opt{info} --outdir $result/../Shell\n";
close RE;
