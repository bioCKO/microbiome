...
=head1 Version
 ...
 BugFix: Zhouhao Zhang, zhangzhouhao@novogene.com
...
=head1 Usage
 ...
 --dvf <srt>        vf cutoff, while --qalter, deault 1g
...
$reqcol,$reqline,$reql,$cycle,$reqtime,$clean,$delwrite,$sure,$equipment,$nodesel,$nodetxt,$reqsub,$novb,
$checke,$focus2,$fsuffix2,$equprefix,$verb,$wrong,$osure,$owrong,$help,$endsh,$noV);
GetOptions(
		"bgrun"=>\$bgrun,
		"mutil"=>\$mutil,
		"workdir:s"=>\$work_dir,
...
##================================##
$bgrun && ($mutil = 1); ##-mutil use to call -bgrun at do_job.pl
$line ||= 1;
$reqtime ||= 30;
$cycle ||= 1;
...
$maxvf ||= '1g';
$maxvf = real_bit($maxvf);#sub3.2.1.4.1
($sure || $wrong) && ($checke = 1);
##=== get qsub commend
my @cp_node;
...
my ($Bdir_new, $nodetxt0) = get_pathway("$Bin/../../bin/Pathway_cfg.txt",[qw(B_dir NODE_text)]);
(-d $Bdir) || ($Bdir = $Bdir_new);
...
#the sub function for qsub or mutil the tasks
{
	...
	while(@run_sh || %err_sh){
		...
		$cycle_num,$qstat,$qstat_pl,$all_num,\$rnum,$cyqt,$qalter,$maxvf,$novb); #sub3.2
...
##===========#
{
	...
	while(@{$run_sh} || %{$err_sh} || $check){
		...
		$reqcol,$reqtime,$reqtime_num,$cpnode,$cycle,$all_num,$rnum,$tc,\$run_record,$qalter,$work_dir,$maxvf,$novb);   #sub3.2.1
		if($able_num <= 0){
			if($able_num == -1){    ### 系统无反应
				($reqtime >= $$reqtime_num)	?
		...
		$rnum,$tc,0,$qstat,$qstat_pl,$qalter,$maxvf,$work_dir,$reqline,$reqcol,$qsub_comm,$cpnode) : #sub3.2.1.4
...
###==============#
{
	...
	if(!$reqsub && $err_sh && %{$err_sh}){
		...
		if($reqtime >= $$reqtime_num){
            my %qstath = ($qstat && -s $qstat) ? split/\s+/,`awk '(NF>4){print \$2,\$5}' $qstat` : ();
			foreach(keys %{$err_sh}){
				...
				if($qalter && $qstath{$_}){
					...
					# bug : vf=70.23G,num_proc=1 => vf=35.45G (num_proc lost)
					# fixed by zhangzhouhao\@novogene.com, 2017-09-19
                    ($maxvmem ne 'N/A') && ($qsub_comm0 =~ s/\bvf=([0-9]+(\.[0-9]+)?[BKMGTbkmgt]?)(,\S*)?/vf=$maxvmem$3/); 
				}
			...
			$all_num,$rnum,$tc,1,$qstat,$qstat_pl,$qalter,$maxvf,$work_dir,$reqline,$reqcol,$qsub_comm,$cpnode) : #sub3.2.1.4
...
####==========#
{
	...
	$stop,$qstat,$qstat_pl,$qalter,$maxvf,$work_dir,$reqline,$reqcol,$qsub_comm,$cpnode) = @_;
	my ($run_id, $new_runsh);
	my $kqdel = $mutil ? 'kill' : 'qdel';
	%{$id_sh} = split/\s+/,$run_record;
	if($stop){
		$new_runsh = $run_record;foreach(keys %{$id_sh}){$run_id .= "$_ ";};
	}elsif(-s $run_shelf){
		...
		if(!$mutil && ($qstat || $qalter)){
			...
			if($qalter && $tem_qstat){
				...
				foreach(`echo "$tem_qstat" | awk '(!/^#/ && !/^-/ && \$1){print \$1,\$2,\$3,\$4,\$5}'`){
					my @l = split/\s+/;
					($l[3] eq 'N/A') && next;
					(real_bit($l[3]) - real_bit($l[2]) > $maxvf) || next;
					my $qsub_comm0 = $qsub_comm;
					#$qsub_comm0 =~ s/\bvf=\S+/vf=$l[4]/;
					# bug : vf=70.23G,num_proc=1 => vf=35.45G (num_proc lost)
					# fixed by zhangzhouhao\@novogene.com, 2017-09-19
					$qsub_comm0 =~ s/\bvf=([0-9]+(\.[0-9]+)?[BKMGTbkmgt]?)(,\S*)?/vf=$l[4]$2/;
					del_oe(@l[1,0]);          ## sub3.2.1.2
					$reqline && `$recolh{$l[1]}`;          ## rewrite the key file
					my $tem_node = node_sel($cpnode);  ## sub 3.2.1.3
					`qdel $l[0]`;
					#my $job_id = (split/\s+/,`$qsub_comm0 $tem_node $work_dir/$l[1]`)[2];
					# bug: $work_dir/$l[1] => $work_dir/$work_dir/$l[1]
					## do not use the absolute directory;
					## fixed by zhangzhouhao\@novogene.com, 2017-09-18
					my $job_id = (split/\s+/,`$qsub_comm0 $tem_node $l[1]`)[2];
...
#####========#
{
	...
	# With BUG : '70G,num_proc=1' => '70G,NUM_PROC=1'(after uc)
	# => '70G,NU'($1, same as 70) + 'M'($2, should be 'G')
	# => 70,000,000 (should be 70,000,000,000)
	# fixed by zhangzhouhao\@novogene.com, 2017-09-19
	($vf=~/^([0-9]+(\.[0-9]+)?)([BKMGT])/) || return($vf);
	my %h = ('B',1,'K',1000,'M',10**6,'G',10**9,'T',10**12);;
	#$1 * $h{$2};
	$1 * $h{$3}; # fixed together with $vf=// bug via zhangzhouhao@novogene.com
}
