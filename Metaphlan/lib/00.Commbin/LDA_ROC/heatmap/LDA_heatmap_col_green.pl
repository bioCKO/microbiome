...
if($group){
    if(!$rank_rank || $outfile=~/cluster\.p/){
        ...
        pheatmap(x,scale=\"row\",color = colorRampPalette(c(\"limegreen\", \"white\",\"red1\"))(100),annotation_col = annotation_col,filename=\"$outfile.pdf\",height=ceiling($pdf_h),width=ceiling($pdf_w),fontsize=20,cluster_cols=FALSE,annotation_colors=ann_colors)    
        dev.off()";
    }else{ 
        print R "library(pheatmap)
        setwd(\"$outdir\")
        ...
        pheatmap(x,scale=\"row\",color = colorRampPalette(c(\"limegreen\", \"white\",\"red1\"))(100),annotation_col = annotation_col, annotation_row = annotation_row,filename=\"$outfile.pdf\",height=ceiling($pdf_h),width=ceiling($pdf_w),fontsize=20,cluster_cols=FALSE,annotation_colors=ann_colors)
        dev.off()";
    }
}else{
    if(!$rank_rank || $outfile=~/cluster\.p/){
        ...
        setwd(\"$outdir\")
        x<-read.table(\"$outfile.txt\",sep=\"\\t\",header=T,row.names=1)
        pheatmap(x,scale=\"row\",color = colorRampPalette(c(\"limegreen\", \"white\",\"red1\"))(100),filename=\"$outfile.pdf\",height=ceiling($pdf_h),width=ceiling($pdf_w),fontsize=20,cluster_cols=FALSE)
        dev.off()";
    }else{
        print R "library(pheatmap)
        setwd(\"$outdir\")
        ...
        pheatmap(x,scale=\"row\",color = colorRampPalette(c(\"limegreen\", \"white\",\"red1\"))(100),annotation_row = annotation_row,filename=\"$outfile.pdf\",,height=ceiling($pdf_h),width=ceiling($pdf_w),fontsize=20,cluster_cols=FALSE,annotation_colors=ann_colors)
        dev.off()";
    }
}
close R;
