...
if($colour_g){
    ...
    open COL ,"< $Bin/group.color.xls ";
	while (<COL>)                                 
	{   
	    chomp ;
	    my $color="\"".(split (/\t/))[0]."\"";
		push (@group_col,$color);
	}   
	close COL;
...
if($group){####add cellheight =20,cellwidth=20  by zhanghao 20180118
    ...
    if(!$rank_rank || $outfile=~/cluster\.p/){
        ...
        setwd(\"$outdir\")
        x<-read.table(\"$outfile.txt\",sep=\"\\t\",header=T,row.names=1)
        pheatmap(x,scale=\"row\",color = colorRampPalette(c(\"royalblue1\", \"white\",\"salmon\"))(100),filename=\"$outfile.pdf\",height=ceiling($pdf_h),width=ceiling($pdf_w),cellheight =20,cellwidth=20,fontsize=20,cluster_cols=FALSE)
        dev.off()";
    }else{
        print R "library(pheatmap)
        setwd(\"$outdir\")
