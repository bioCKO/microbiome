...
my ($combine,$rscript,$svg2xxx,$convert) = get_pathway("$Bin/../../../../../bin/Pathway_cfg.txt",[qw(COMB_FIG R7 SVG2XXX CONVERT)],$Bin,$lib); ##old $rscript is Rscript
...
while (<OR>) {
    ...
    close OUT;
my $namelen=length($id);
my $cutname;
if ($namelen>60){
    $cutname=substr($id,0,60). "(\.\.\.)";
}else{$cutname=$id}
    ...
    close OUT;
    system "$rscript -f '$opt{outdir}/R/$id.R'";
...
my $top_num;
my $toppics;
open LIST,">$opt{outdir}/sorted.list";
foreach (sort{$b <=> $a} keys %max2id){
    print LIST join(",",@{$max2id{$_}}),",";
}
...
system  "$combine $toppics >$opt{outdir}/top.$opt{top}.svg1";
system "perl -ne \'chomp;\$a=\$_;\$a=~s/&/&amp;/g;print \"\$a\\n\";\' $opt{outdir}/top.$opt{top}.svg1 >$opt{outdir}/top.$opt{top}.svg ";
system "rm $opt{outdir}/top.$opt{top}.svg1";
