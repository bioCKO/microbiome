...
($opt{dir} && -s $opt{dir} && $opt{vs} && -s $opt{vs})||die "Usage: perl $0
    ...
    *--vs       [str]    input the vs.list
    ...
    --colour    [str]    choose colour for groups,default='#00CC00','#FF6666'
    --sep                if only have 2 group,whether to plot increase and decrease tax separately,defaute not to set it\n";

use lib "$Bin/../../../";
my $lib = "$Bin/../../../..";
use PATHWAY;
(-s "$Bin/../../../../../bin/Pathway_cfg.txt") || die"error: can't find config at $Bin/../../../../../bin, $!\n";
my ($rscript,$svg2xxx,$convert) = get_pathway("$Bin/../../../../../bin/Pathway_cfg.txt",[qw(Rscript SVG2XXX CONVERT)],$Bin,$lib);
...
if($opt{colour}){
    ...
    @color = ('"#00CC00"','"#FF6666"','"#439dee"','"#ee439d"','"#cb4154"','"#1dacd6"','"#66ff00"','"#bf94e4"','"#c32148"','"#ff007f"','"#08e8de"','"#d19fe8"');
}
my $colors;
...
if ($opt{sep} && @vs==1){
    ...
    if(@decrease_tax){
        my $limit1=0;
        open OUT1,">$opt{outdir}/combine_box/decrease_tax.xls";
  BLOCK:foreach my $temp(@sort){
    ...
    if(@increase_tax){
        my $limit=0;
        open OUT2,">$opt{outdir}/combine_box/increase_tax.xls";
  BLOCK:foreach my $temp(@sort){
            $temp =~ /^Others/ && next;
    ...
    $opt{top} ||= 12;
    my $limit=0;
    open OUT,">$opt{outdir}/combine_box/top.$opt{top}\_tax.xls";
BLOCK:foreach my $temp(@sort){
        $temp =~ /^Others/ && next;
        $limit++;
        my $tax = (split /\_/,$temp)[-1];
...
my @plot_file = `ls $opt{outdir}/combine_box/*_tax.xls`;
