...
s1 = sum(pvalues <= threshold);
s2 = sum(qvalues <= threshold);
...
for (i in 1:nrows){
  if (pvalues[i] <= threshold){
    ...
    p_Differential_matrix[dex,2:4] = C1[i,];
    p_Differential_matrix[dex,5:7] = C2[i,];
    p_Differential_matrix[dex,8]   = pvalues[i];  
    p_Differential_matrix[dex,9]   = qvalues[i];
    dex = dex+1;  
  }
}
for (i in 1:nrows){
  if (qvalues[i] <= threshold){
    q_Differential_matrix[dex2,1]   = jobj$taxa[i];
    q_Differential_matrix[dex2,2:4] = C1[i,];
    q_Differential_matrix[dex2,5:7] = C2[i,];
    q_Differential_matrix[dex2,8]   = pvalues[i];
    q_Differential_matrix[dex2,9]   = qvalues[i];
    dex2 = dex2+1;
  }
}


colnames(p_Differential_matrix) = c("Taxa","mean(group1)","variance(group1)","standard error(group1)","mean(group2)","variance(group2)","standard error(group2)","p value","q value")
colnames(q_Differential_matrix) = c("Taxa","mean(group1)","variance(group1)","standard error(group1)","mean(group2)","variance(group2)","standard error(group2)","p value","q value")
...
colnames(Total_matrix) = c("Taxa","mean(group1)","variance(group1)","standard error(group1)","mean(group2)","variance(group2)","standard error(group2)","p value","q value")
...
#show(Differential_table)
#write(t(Differential_matrix),output2,ncolumns = 9, sep = "\t");write.table(Differential_table,file=output2,row.names = F,sep = "\t");
#show(Total_table)
...
write.table(q_Differential_table,file=output3,row.names = F,quote = FALSE,sep = "\t");
}
