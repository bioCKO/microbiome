#! /usr/bin/perl
# Function: transfer metaphlan_table.even.txt to the file lefse can accepted
# Data: 2014-06-12, Version 1.0, input is metaphlan_table.relative.mat
...
use warnings;
if (@ARGV < 3) {
	die "usage: perl $0 <input:Relative mats folder> <input:all.mf> <output: for lefse input> [vs_group:group1 group2]\n";
}
my($metaphlan_table,$mf,$output,@vs_group)=@ARGV;
my (%mf,@vs_samples);
...
while (my $or=<MF>) {
	...
	my @or=split(/\s+/,$or);
	$mf{$or[0]}=$or[1];
    $sample_num=`grep -w  $or[1] $mf | wc -l`;
...
foreach(`ls $metaphlan_table/metaphlan.[cfgkops].relative.xls`){
	chomp;
	open(OR,"$_");
	my $head=<OR>;
	chomp $head;
