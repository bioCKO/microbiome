...
c=c()
ncols = ncol(Pmatrix)
#allsame=array(0, dim=c(0,ncols+1));
  
for(i in 1:dim(Pmatrix)[1]){
if( (sum(Pmatrix[i, 1:(g-1)])==0) & (sum(Pmatrix[i, g:ncols])==0) ){
    c= append(c,-i)
 }else if(  (length(unique(Pmatrix[i, (1:g-1)]))==1) & (length(unique(Pmatrix[i, g:ncols]))==1) ){ #samples in group has same abundance 
#temp = c(taxa[i],Pmatrix[i,])
#     allsame = rbind(allsame,temp) #save as a file
      c= append(c,-i)#delete this taxa
  }
}
#if(dim(allsame)[1]>0){
# allsame=as.data.frame(allsame)
# allsamefile = paste(output,"same_group.xls",sep="")
# write.table(allsame,file=allsamefile,row.names = F,quote = FALSE,sep = "\t");
#}
