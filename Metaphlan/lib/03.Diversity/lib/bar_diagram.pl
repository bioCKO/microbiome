...
my ($height,$width, $flank_x,$flank_y, $size_x,$micro_scale,
    ...
    $preci,$help,$h,$barw,$y_mun,$h_pos,$size_ht,$row,$sign_dis,
...
qw(crimson blue lightseagreen orange mediumpurple palegreen lightcoral dodgerblue lawngreen olive
    yellow fuchsia salmon mediumslateblue darkviolet purple sienna tan chocolate skyblue turquoise cadetblue silver aliceblue  black );
...
if ($right){
    ...
    $sign_dis ||= $pwidth-$width-$flank_x-2*$size_sig;
    $size_sig_max = $sign_dis / $rig_str_len / 0.6;
    if($size_sig_max < 10){
        $pwidth += (10-$size_sig_max)*$rig_str_len*0.6;
        $size_sig_max = 10;
    }
