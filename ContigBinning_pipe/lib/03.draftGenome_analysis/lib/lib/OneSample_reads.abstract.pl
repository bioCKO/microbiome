...
for(`less $DataList`){
    ...
    open IN1, "<:gzip", $fq1 || die $!;
    open IN2, "<:gzip", $fq2 || die $!;
    while(my $info1 = <IN1>,my $info2 = <IN2>){
        my $readID = (split /\s+/,$info1)[0];
        $readID =~ s/\/\d$//g; $readID =~ s/^\@//;
        $info1 .= <IN1> . <IN1> . <IN1>;
        $info2 .= <IN2> . <IN2> . <IN2>;
        $sample2fq{$sampleID}{$readID}{1} = $info1;
        $sample2fq{$sampleID}{$readID}{2} = $info2;
    }
    close IN1;close IN2;
}
#close F;
...
while(<F>){
     ...
     foreach(split/\n/, `gzip -dc $fq1 |grep \'^@\'`){
         ...
         if(exists $sample2fq{$bin2sample{$bin}}{$readID}){
             print O1 "$sample2fq{$bin2sample{$bin}}{$readID}{1}";
         }
     }
     foreach(split/\n/, `gzip -dc $fq2 |grep \'^@\'`){
         my $readID = $_;
         $readID =~ s/\/\d$//g; $readID =~ s/^\@//;
         if(exists $sample2fq{$bin2sample{$bin}}{$readID}){
             print O2 "$sample2fq{$bin2sample{$bin}}{$readID}{2}";
         }
     }
