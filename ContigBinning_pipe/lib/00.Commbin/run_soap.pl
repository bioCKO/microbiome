...
if(-s "$index_lib.index.sai"){
    ...
    close BSH;
	system"cd $shdir;$super_worker -resource $dvf -splitn 1 -prefix bwt bul_bwt.sh";
    BWT:{;}
	$index_lib .= ".index";
}
...
my $line = 0;
my $gzip2 = $gzip ? 1 : 0;
($revi_file || $step=~/3/) || ($opt{resoap} = 0);
($run_cover || $revi_file || $step=~/2|3/ || $opt{sortsoap} || $opt{resoap}) && ($gzip = 0);
$gzip && ($gzip2 = 0);
...
my $mask_sign = $opt{resoap} ? "#" : " ";
my $old_index = $index_lib;
...
while(<IN>){
	...
	if (/L\d+_([^_]+)\.notCombined_[12]\.f[aq]/ || /L\d+_([^_]+)_[12]\.fq/) {
		...
		print OUT "cd $soap_dir\n$soap_path $soapopts -D $index_lib $soap_opt_cur -o $bname.PE.soap -2 $bname.SE.soap 2> $bname.log\n";
		if($opt{incomp_del}){
		...
		if($step=~/5/){
			print OUT "$mask_sign cd $insert_dir\n$mask_sign $soap_insert $soap_dir/$bname.{PE,SE}.soap\n";
		}
#   print OUT ($gzip ? "gzip $bname.PE.soap & gzip $bname.SE.soap & wait\n\n" : "\n");
		print OUT ($gzip ? "cd $soap_dir\ngzip $bname.PE.soap\ngzip $bname.SE.soap\n\n" : "\n");
	}
		...
		print OUT "cd $soap_dir\n$soap_path $soapopts -D $index_lib $soap_opt_cur -o $bname.SE.soap 2> $bname.log\n";
		if($opt{incomp_del}){
...
#    `cd $shdir;$super_worker -resource $resource -splits \"$splits\" $run_soap_sh -prefix soap -clean`;
    `cd $shdir;$super_worker -resource $resource -splits \"$splits\" $run_soap_sh -prefix soap`;
    `ls $soap_dir/*.$suffix >$outlst`;
    $verbous && (print STDERR localtime() . " --> finesh runnint soap\n");
    $opt{resoap} && ($soap_dir2 .= 2);
    if($opt{sortsoap}){
        $opt{prefix} ||= 'all';
        open SOR,">$shdir/sortSoap.sh" || die"$!";
        open SLS,">sortsoap.lst";
        print SOR "cd $soap_dir2; cat ./*.soap | $Bin/msort -k 'm8,n9' > $opt{prefix}.sort\ndate > $shdir/sort.frec\n";
#print SOR "cd $soap_dir2; less ./*.soap | $Bin/msort -k 'm8,n9' > $opt{prefix}.sort\ndate > $shdir/sort.frec\n";
...
if(!($run_cover || $revi_file || $step =~ /[234]/)){
    ($step=~/1/ && $opt{sortsoap}) && `perl -e 'until(-s \"$shdir/sort.frec\"){sleep(1);}'`;
    exit(0);
}
$index_lib =~ s/\.index$//;
...
###  step4 run GC depth    ========================================================================================
    ...
    if(!(($revi_file && $step =~/3/) || ($opt{resoap} && $revi_file && -s $revi_file)) && $gzip2){
        print SOC "$mutil_process $shdir/gzip.sh\n";
        $gzip2 = 0;
    }
    close SOC;
...
### rerun soap  ===============================================================================================
if($opt{resoap} && $revi_file && -s $revi_file){
    (-d $indir) || mkdir($indir);
    (-s $indir) && `rm -r $indir/*`;
    `ln -s $revi_file $indir`;
    (-d $soap_dir2) || mkdir($soap_dir2);
    my $revi_bname = (split/\//,$revi_file)[-1];
    my $new_index = "$indir/$revi_bname.index";
    open IN,"$shdir/$run_soap_sh" || die$!;
    open OUT,">$shdir/rerun_soap.sh" || die$!;
    while(<IN>){
        s#^\###;
        s#-D $old_index#-D $new_index#g;
        s#$soap_dir#$soap_dir2#g;
        print OUT;
    }
    close IN;
    close OUT;
    open SH,">$shdir/run_rerun_soap.sh" || die$!;
    print  SH "cd $indir\n$bwt_builder $revi_bname\n",
    "cd $shdir;$super_worker -resource $resource -splits \"$splits\" rerun_soap.sh -prefix resoap -clean\n";
    print SH ($run ne 'qsub') ? "cd $shdir;sh sortSoap.sh \n" :
    "cd $shdir;$super_worker -resource $resource -splitn 1 sortSoap.sh \n";
...
($step=~/1/ && $opt{sortsoap}) && `perl -e 'until(-s \"$shdir/sort.frec\"){sleep(1);}'`;
if($opt{resoap}){
    `perl -e 'until(-s \"$shdir/resoap.frec\"){sleep(1);}'`;
    system"rm -r $soap_dir;mv -r $soap_dir2 $soap_dir";
}
if($gzip2){
