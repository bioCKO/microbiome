...
#  --large <int>  1: input is large insert size soap result, 0: isn't large insert size soap result.
...
use PerlIO::gzip;
use Getopt::Long;
my ($prefix,$nodraw,$large);
GetOptions("prefix:s"=>\$prefix,"nodraw"=>\$nodraw,"large:i"=>\$large);
my $usage=<<USAGE;
...
#			$insert = $x2 - $x1 + $len2;
				$insert = $x2 - $x1;
	            #($insert > 0) ? ($insert += $len2) : ($insert > -90) ? ($insert = $len1 - $insert) : ($insert -= $len1);
    	        ($insert > 0) ? ($insert += $len2) : ($insert > -$len2) ? ($insert = $len1 - $insert) : ($insert -= $len1);
				$insert{$insert}++;
			}
			elsif ($f2 eq '+' && $f1 eq '-') {
#			$insert = $x1 - $x2 + $len1;
				$insert = $x1 - $x2;
            	#($insert > 0) ? ($insert += $len1) : ($insert > -90) ? ($insert = $len2 - $insert) : ($insert -= $len2);
            	($insert > 0) ? ($insert += $len1) : ($insert > -$len1) ? ($insert = $len2 - $insert) : ($insert -= $len2);
				$insert{$insert}++;
			}
		}
		elsif ($large) {
...
if($nodraw){
    my $libname = ($input[0] =~ /L\d+_([^_]+)\.notCombined_[12]\.f[aq]/ || $input[0] =~ /L\d+_([^_]+)_[12]\.fq/) ? $1 : 'NA';
...
for (my $i=0; $i<@insert; $i++) {
	if($insert[$i]>-40000&&$insert[$i]<40000)
	{
